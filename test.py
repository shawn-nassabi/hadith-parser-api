...
5) Normalizes keys, compares extracted sanad vs validation dataset
6) Uses deterministic token-subset matching, then LLM fallback ("YES/NO/PROPHET")
7) Ignores Prophet mentions in metrics, logs unmatched, prints narrator_name_accuracy/unique_narrator_name_accuracy/F1
"""
...
def llm_name_match(extracted: str, val_ds_narr_names: List[str]) -> str:
	"""
	...
	print("validation dataset narrator names:", val_ds_narr_names)

	prompt = (
		"You are an expert in Arabic names, specifically in the context of Arabic Ahadith narrators (sanad). "
		"Compare the following extracted narrator name with the list of validation dataset narrator names."
		...
		"Sometimes the last name of a person might also be included in the extracted name but not in the validation dataset (or vice versa) but that is fine, answer with 'YES'"
		...
		f"Vaidation dataset: {val_ds_narr_names}\n\n"
...
with open(output_file_1, "w", encoding="utf-8") as f1, open(output_file_2, "w", encoding="utf-8") as f2:
	...
	f1.write(header)
	f2.write(header)
	total_num_hadith = len(results)
	sequence_num = 0


	predicted_narr_names_unique = {}
	validation_ds_narr_names_unique = {}
	for api_result in results:
		...
		# Parse validation dataset narrator IDs -> names
		try:
			narrator_ids = [int(i) for i in safe_str(chain_str).split(',') if i.strip().isdigit()]
		except Exception:
			narrator_ids = []
		val_ds_narr_names = [get_rawi_name(i) for i in narrator_ids]
		...
		for idx, name in enumerate(extracted_sanad):
			if name in predicted_narr_names_unique:
				predicted_narr_names_unique[name] += 1
			else:
				predicted_narr_names_unique[name] = 1
			ex_tokens = safe_tokenize(name)
			found = False

			for full_name in val_ds_narr_names:
